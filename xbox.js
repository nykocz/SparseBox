class BxLogger {
  ...
  remotePlayServerId: (window.BX_REMOTE_PLAY_CONFIG && window.BX_REMOTE_PLAY_CONFIG.serverId) || '',`;
      ...
      remotePlayDisableAchievementToast(str) {
        ...
        if (!str.includes(text)) return !1;
        let newCode =
          "if (window.location.pathname.includes('/consoles/launch/')) return;";
  ...
  `;
      ...
      patchCustomInputIcon(str) {
        ...
        if (index < 0) return !1;
        let productIdMatch = /const (\w+)=(\w+)=>{/.exec(
          str.substring(index, index + 200)
        );
        if (!productIdMatch) return !1;
        str = str.replace(
          productIdMatch[0],
          productIdMatch[0] + `const productId = ${productIdMatch[2]};`
        );
        ...
        if (
          ((str = str.replace(
            match[0],
            `(${match[1]} || window.BX_EXPOSED.hasCustomTouchControl(productId)) && ${match[2]}`
          )),
          ...
          str = str.replace(
            match[0],
            `(${match[1]} || window.BX_EXPOSED.hasCustomNativeMkb(productId)) && ${match[2]}`
          );
        return str;
      },
    },
    ...
    getSignature() {
      let scriptVersion = SCRIPT_VERSION,
        ...
        $link = document.querySelector(
          'link[data-chunk="client"][as="script"][href*="/client."]'
        );
      if ($link) {
  ...
  if (BX_FLAGS.SafariWorkaround && document.readyState !== "loading") {
    ...
    let $fragment = document.createDocumentFragment();
    throw (
      ($fragment.appendChild(CE("style", !1, css)),
      $fragment.appendChild(
        CE(
          "div",
          { class: "bx-reload-overlay" },
          CE("div", !1, CE("p", !1, t("load-failed-message")), $secondaryAction)
        )
      ),
      document.documentElement.appendChild($fragment),
      isSafari && window.location.reload(!0),
      new Error("[Better xCloud] Executing workaround for Safari"))
    );
  }
